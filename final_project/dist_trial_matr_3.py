...
def dist(list_of_in):
    ...
    for x in range(0,len(nucl_freq_list)):
        for y in range(0,len(nucl_freq_list)):
            mat_dist[x,y]=nucl(1000 * nucl_freq_list[x], 1000 * nucl_freq_list[y])
    print("solo nucl\n",mat_dist)
    np.savetxt("nucl_freq_dist_matrix_1000.grimm",mat_dist,fmt="%.5f")
    ...
    for x in range(0,len(nucl_freq_duo)):
        for y in range(0,len(nucl_freq_duo)):
            mat_dist_duo[x,y]=nucl(1000 * nucl_freq_duo[x], 1000 * nucl_freq_duo[y])
    print("duo nucl\n",mat_dist_duo)
    np.savetxt("dinucl_freq_dist_matrix_1000.grimm",mat_dist_duo,fmt="%.5f")
